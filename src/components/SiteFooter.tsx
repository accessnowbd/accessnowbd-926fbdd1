...
export function SiteFooter() {
  return (
    <footer className="relative mt-16 border-t border-white/10 bg-[rgba(8,5,24,0.6)] backdrop-blur-2xl">
      ...
      <div className="mx-auto max-w-[1440px] px-4 md:px-10 py-14 grid md:grid-cols-5 gap-10">
        <div className="md:col-span-2">
          <div className="flex items-center gap-2.5 font-bold text-white text-lg" style={{ fontFamily: "var(--font-heading)" }}>
            <span className="relative grid place-items-center w-10 h-10 rounded-2xl bg-aurora text-white">
          ...
          <p className="text-xs uppercase tracking-[0.25em] text-aurora-strong font-bold mt-3">Fast • Secure • Reliable</p>
          ...
          <div className="mt-5 flex gap-2">
            {[Facebook, Instagram, Youtube, Twitter].map((Icon, i) => (
              <a key={i} href="#" className="grid place-items-center w-9 h-9 rounded-full bg-white/5 border border-white/10 hover:bg-white/15 hover:border-white/20 transition" aria-label="social">
                <Icon className="w-4 h-4 text-white/80" />
              </a>
            ))}
          </div>
          <a href="https://wa.me/8801000000000" className="mt-4 inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-[#25D366] text-white text-sm font-semibold hover:scale-[1.02] transition shadow-[0_8px_30px_-8px_rgba(37,211,102,0.6)]">
        ...
        <div>
          <h4 className="text-sm font-bold text-white mb-3">Get in touch</h4>
          <ul className="space-y-2.5 text-sm text-white/60">
            <li className="flex items-center gap-2"><Mail className="w-3.5 h-3.5 text-aurora" /> accessnowbd01@gmail.com</li>
            <li className="flex items-center gap-2"><Phone className="w-3.5 h-3.5 text-aurora" /> +880 1000 000000</li>
            <li className="flex items-center gap-2"><MapPin className="w-3.5 h-3.5 text-aurora" /> Dhaka, Bangladesh</li>
          </ul>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-[1440px] px-4 md:px-10 py-5 flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-white/50">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {["bKash", "Nagad", "Rocket", "Visa", "Mastercard"].map((p) => (
              <span key={p} className="px-2.5 py-1 rounded-md bg-white/5 border border-white/10 text-white/70 text-[10px] font-semibold tracking-wide">{p}</span>
            ))}
          </div>
        </div>
      </div>
